...
Theory of plain operads is an example of an infinitary algebraic theory. For every natural number `n` we have a sort `Op[n]` of the theory. For every
list `n1,...,n_l` (including an empty list) of natural numbers we have an operation
```
    x1 : Op[n1] .... x_l : Op[n_l]
—————————————————————————————————————————
 comp(x1, ..., x_l) : Op[n1 + ··· + n_l]
```

Besides the operations, we have a number of equations rendering this operations coherent. The rules define a category `C` (“the walking operad”), objects of which are lists of natural numbers, maps are generated by the operations as defined above module equations. An operad is an image of this category under a product-preserving functor.

Note that the sorts, operations and equations of the theory can be described as a free model of a finitary algebraic theory. What if we could somehow use it to give a finitary presentation of this algebraic theory? Instead of an infinite number of sorts `Op[n]` we'd have a single sort `Op` fibered over the sort of natural numbers. The sort of natural numbers would not belong to the theory proper but be an auxiliary sort (required to be modeled `free` in all models, not just the free ones). We'll denote that as follows:
...
Since, every natural number is a map `n : 1 -> Nat`, so the (infinite number of) sorts `Op[n]` can be recovered as pullbacks `Op ×₍arity = n₎ 1`.

Now, in order to describe operations, we need a single type `List` instead of `x1 : Op[1], x2 : Op[2],...`:
```
                               head : Op   tail : List
——————————    ————————————    —————————————————————————
 List aux      nil : List        (tail; head) : List
```

Now we almost can write down the composition operation:
```
    c : List
——————————————
 comp(c) : Op
...
Yet whenever we provide a constructor for `Op` we actially should tell what the arity, and for that we need to calculate the total arity of the context:
```
               c : List                                        head : Op   tail : List
——————————    ——————————    ———————————————————————     ——————————————————————————————————————————————————————
 List aux      c.arity       nil : List[arity = 0]       (tail; head) : List[arity = head.arity + tail.arity]
 
         c : Ctx
———————————————————————————————
 comp(c) : Op[arity = c.arity]
```

Here we used addition of natural numbers “+” without having defined it. We could define


To provide equational laws similar notion of computation will be required again: we'll provide a type of trees (expressions built from compositions) and two computations on them: fold (fold by composition operations) and flatten (into a list). All coherency laws will be generated by
```
      t : Tree 
——————————————————————————
 t.fold = comp(t.flatten)
```
