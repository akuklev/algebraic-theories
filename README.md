...
Now, in order to describe operations, we need a single type `Ctx` instead of `x1 : Op[1], x2 : Op[2],...`:
```
                             head : Op   tail : Ctx
—————————    ———————————    ————————————————————————
 Ctx aux      nil : Ctx        (tail; head) : Ctx
```
...
```
              c : Ctx                                        head : Op   tail : Ctx
—————————    —————————    ——————————————————————    —————————————————————————————————————————————————————
 Ctx aux      c.arity      nil : Ctx[arity = 0]      (tail; head) : Ctx[arity = head.arity + tail.arity]
