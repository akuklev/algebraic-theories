...
Inductive types may have typal parameters. Such types are called polymorphic. For example, the type `List[T]` of lists parametrized by the type `T` of its elements is defined as follows:
```
#Inductive List[\T]
  Empty : List[T] 
  (::)  : T → (List[T] → List[T])
