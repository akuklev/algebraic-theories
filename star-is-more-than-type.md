...
Here the type `Nat` is declared to be a type freely generate by one non-recursive generator `0` satisfying type `Nat` and a recursive generator `(_')` satisfying type `Nat → Nat`. With this definition, the set of possible Nat-values is given by `0, 0', 0'', 0''', etc`.
...
The indices itself are freely generated by generators `0` and `(_')` just as natural numbers. But additionally there are right arrows `|Extend(m)⟩` going _from_ to each element `[n]`. Together with arrow source `[n]` the argument `m` of the arrow generator `|Extend(m)⟩` allows to compute which element the arrow goes _to_. Right arrows are required to go from structurally smaller elements to structurally larger ones, that is they comply with natural lexicographic ordering on the inhabitants of `NatBound`. For each pair of composable arrow generators we have to provide the composition (last line of the definition above), which has to be strictly associative. Left arrows in indexes are called embeddings.
