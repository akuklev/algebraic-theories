...
Inductive types may also be polymorphic, i.e. have parameters. For example the type `List[T]` requires a typeformer `T` satisfying the signature `*` which roughtly means “a type”. In particular the typeformer `Nat` satisfies this signature: `Nat : *`, so we can generate the type `List[Nat] : *`, and therefore also the type `List[List[Nat]]` and so on. The signature of the typeformer `List` itself is writen `* → *`, which follows the intution that `List` acts like a function mapping types to types, apart from the particularity that a type former is not exactly a function. Now let's write down the definition:
```
#Inductive List[\T : *]
  Empty : List[T]
  (::)  : T → List[T] → List[T]
