...
Here the type `Nat` is introduced as type freely generate by one non-recursive generator `0` satisfying type `Nat` and a recursive generator `(_')` (writen as a postifx apostrophe) satisfying type `Nat → Nat`. With this definition, the set of possible Nat-values is given by `0, 0', 0'', 0''', etc`. 

Inductive types may also be polymorphic, i.e. have typal parameters. Let us define the type `List[T]` of lists parametrized by the type `T` of its elements:
```
#Inductive List[\T]
  Empty : List[T] 
  (::)  : T → (List[T] → List[T])
...
The first line introduces `List` as a polymorphic inductive type with with a typal parameter `T`: typal parameters are enclosed in square brackets, the backslash in `\T` is the so called “freshness sigil”, it underlines that `T` is not a constant that was defined somewhere above, but a fresh variable being introduced at this precise spot.

The second line introduces a rather boring generator `Empty` of `List[T]`. The third line is way more iteresting. The generator introduced there is given by an infix operator `::`, and it is not a single generator, but a family of generators: for each `\head : T` we'll have a recursive generator
`(head ::) : List[T] → List[T]` that appends `head` to the right hand side argument `\tail : List[T]` so one can generate lists by succesively appending elements to the `Empty` list. The set of possible values of  `List[T]` is therefore given by
```
Empty
x :: Empty
x :: y :: Empty
...
Note that the arrow operator is right-gathering, so we will be writing `T → List[T] → List[T]` instead of `T → (List[T] → List[T])`
...
```
#Inductive FinNatVec[\b : NatBound]:
   Empty    : FinNatVec[0]
   (::)[\b] : Nat → FinNatVec[b] → [b']
