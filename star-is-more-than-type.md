...
Inductive types may also be polymorphic, i.e. have parameters. For example the type `List[T]` requires a typeformer `T` satisfying the signature `*` which roughtly means “a type”. In particular the typeformer `Nat` satisfies this signature: `Nat : *`, so we can generate the type `List[Nat] : *`, and therefore also the type `List[List[Nat]]` and so on. The signature of the typeformer `List` itself is writen `* -> *`, the arrow should be understood as “turns a type into type”. Now let's write down the definition:
```
#Inductive List[\T : *]
  Empty  : List[T]
